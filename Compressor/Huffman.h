...
uint8_t* huffman ( uint8_t *dados, int tamanho, int *novo_tamanho = nullptr )
{
    ...
    auxiliar = std::vector<No_arvore*>( ordem_no );
    ...
    uint8_t *retorno = new uint8_t[ dados_retorno.size() + sizeof(int) ];
    memcpy( retorno, &tamanho, sizeof(int) );
    memcpy( &retorno[sizeof(int)], dados_retorno.data(), dados_retorno.size() );
    
    deletar_arvore( raiz );

    if ( novo_tamanho != nullptr )
        *novo_tamanho = dados_retorno.size() + sizeof(int);
...
uint8_t* huffman ( Pixel *dados, int tamanho, int *novo_tamanho = nullptr )
{
    ...
    memcpy( retorno, &tamanho_dados, sizeof(int) );
    memcpy( &retorno[sizeof(int)], dados_retorno.data(), dados_retorno.size() );
...
uint8_t* huffman_i ( uint8_t *dados, int *novo_tamanho = nullptr )
{
    ...
    memcpy( &tamanho_original, dados, sizeof(int) );
    uint8_t *retorno = new uint8_t[ tamanho_original ];
    No_arvore *percorrer;
    uint8_t mascara = 0b10000000;
    uint8_t bit_lido;
    int pos_retorno = 0;
    int pos_dados = sizeof(int);
    ...
    do {
        memcpy( &repeticoes, &dados[ pos_dados ], sizeof(int) );
        pos_dados += sizeof(int);
        if ( repeticoes != 0 ) {
            cor = dados[ pos_dados++ ];
